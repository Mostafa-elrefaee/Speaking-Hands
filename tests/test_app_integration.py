...
cv2 drawing, GesturePipeline, PredictionStabilizer/GestureStabilizer,
SpeechWorker (TTSWorker + in-process pyttsx3 backend with a fake engine).
...
def workdir(tmp_path, monkeypatch):
    ...
    def is_repo_module(m):
        return (m in ("app", "model", "landmark_utils", "gesture_output", "utils",
                      "camera_utils", "dataset_utils", "sh_research")
                or m.startswith("model.") or m.startswith("sh_research."))
    for m in list(sys.modules):
        if is_repo_module(m):
            monkeypatch.delitem(sys.modules, m, raising=False)
    before = set(sys.modules)
    yield dst
    # Repo modules imported DURING the test were bound to the fake tensorflow /
    # mediapipe and to the temp copy; drop them so later tests re-import the
    # real ones (monkeypatch only restores entries that existed before).
    for m in list(sys.modules):
        if m not in before and is_repo_module(m):
            del sys.modules[m]
...
def run_app(monkeypatch, frames, stable_frames=4, no_tts=False,
    ...
    class RecordingSpeechWorker(SpeechWorker):
        def __init__(self, engine_factory=None, **kw):
            super().__init__(engine_factory=lambda: engine, **kw)
    ...
    # --target_fps 0 = the original synchronous every-frame loop, so the
    # scripted frames are processed one-by-one (deterministic).
    argv = ["app.py", "--stable_frames", str(stable_frames),
            "--seq_model", seq_model, "--target_fps", "0"]
    if no_tts:
        argv.append("--no_tts")
